...
// CreateUserBalance .
func (ub UserBalanceRepo) CreateUserBalance(ctx context.Context, u *biz.User) error {
	var userBalance UserBalance
	userBalance.UserId = u.ID
	res := ub.data.DB(ctx).Table("user_balance").Create(&userBalance)
	if res.Error != nil {
		return errors.New(500, "CREATE_USER_BALANCE_ERROR", "用户余额信息创建失败")
	}
...
// Exchange .
func (ub *UserBalanceRepo) Exchange(ctx context.Context, userId int64, amount int64, amountUsdtSub int64, amountUsdt int64, locationId int64) error {
	var (
		err error
	)
...
// GreateWithdraw .
func (ub *UserBalanceRepo) GreateWithdraw(ctx context.Context, userId int64, relAmount float64, amount float64, coinType string) (*biz.Withdraw, error) {
	var withdraw Withdraw
	withdraw.UserId = userId
	withdraw.AmountNew = amount
	withdraw.AmountNewRel = relAmount
